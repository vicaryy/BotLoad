...
public class TwitterDownloaderInfo {
    ...
    private final String fileTooBigExplanation = MarkdownV2
            .apply("Sorry but i can upload files up to 50MB (Telegrams fault).\nThings may change in time.")
            .get();
    ...
    private final String multiVideoExplanation = MarkdownV2
            .apply("This Twitter link contains multiple videos, you need to specify which one you want to download." +
    ...
    private final String receivedWrongNumberInMultiVideo = MarkdownV2.apply("No video in multi-video Twitter link!").toBold().newlineAfter(2).get();

    public String getReceivedWrongNumberInMultiVideo(int amountOfFiles, int specifyNumber) {
        final String explanation = MarkdownV2
                .apply(String.format("Amount of videos in your link: %d" +
                                     "\nThe video number you have specified: %d", amountOfFiles, specifyNumber))
                .newlineAfter()
                .get();
        return receivedWrongNumberInMultiVideo + explanation;
    }
}
